...
def test_first_name_change(shared):
    shared.changeFirstName("brendan", {'from':accounts[0]})
    assert shared.getUserFirstName(accounts[0], {'from':accounts[0]}) == 'brendan', "first name was not updated"

def test_last_name_change(shared):
    shared.changeLastName("birch", {'from':accounts[0]})
    assert shared.getUserLastName(accounts[0], {'from':accounts[0]}) == 'birch', "last name was not updated"